---
title: "Principal Components Analysis Exercise"
author: "Albert Y. Kim"
...
and `X_recenter` (Note for students who have taken MATH 310 Probability: they are
...
* Compute the first and second principal components and plot them in red and
blue respectively.
* Linear algebra fact: they are the first and second *eigenvectors*
$\vec{\gamma}_1$ & $\vec{\gamma}_2$ of the covariance matrix.
...
length <- 3
eigen_vec_1 <- length*Gamma[, 1]
eigen_vec_2 <- length*Gamma[, 2]
PC1_orig <- data_frame(
...
most of the "action" of the 50 points is along the direction of the red arrow.
...
Intuitively speaking, there is less "action" along the direction of the blue arrow.
* The two principal components are perpendicular to each other.
* Linear algebra fact: The red and blue vectors form an *orthogonal basis*.
...
Let's compare how the 50 points in original space $(X_1, X_2)$ look in principal
components space $(Y_1, Y_2)$:
...
* The principal components space $(Y_1, Y_2)$ is simply a rotation of the
original space $(X_1, X_2)$ of about `r 180 + 360*atan(Gamma[2,1]/Gamma[1,1])/(2*pi)` 
degrees clockwise.
...
the $\vec{Y}$ (the sum along both diagonals = `r diag(cov(X)) %>% sum() %>% round(3)`).
