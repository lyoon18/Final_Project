...
As you can see from above, the original dataset was structured in that each row represented an ingredient in a recipe. This format makes it very difficult for us to run classification tree models so we decided to convert all the ingredients into binary indicators using OneHot Encoding.
...
freq <- sort(table(food$ingred), decreasing = T)
freq <- freq[1:80]
...
```
As the wordcloud shows, the most commonly used ingredient is salt. 
