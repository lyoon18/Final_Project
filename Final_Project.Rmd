...
```{r}
fit_large_graph2 <- as.data.frame(trf) %>% gather(key = guess, value = percent, 2:20)
...
```{r}
fit_large_graph3 <- t %>% gather(key = guess, value = percent, 2:ncol(t))
...
dumby2 <- dumby[,c(-2)]
all.equal(rownames(fitC_importance), colnames(dumby2))
